  ...
  .exec((err, contest) => {
    if (err) handler(false, 'Failed to load contest.', 503)(req, res);
      ...
      if (contest.competition.czars.indexOf(req.user._id.toString()) === -1 &&
        ...
        const testCopy = Object.assign(new Test(), {
          name, num_problems, contest
        });
        test.save(err => {
          console.log(err);
          if (err) handler(false, 'Failed to save test.', 503)(req, res);
            ...
            contest.save(err => {
              if (err) {
                console.log(err);
                handler(false, 'Failed to save test to contest.', 503)(req, res);
              }
              else {
                // hacky workaround so there's no circular issues with .save()
                const testCopy = Object.assign(
                  new Test(), {name, num_problems, contest: contest_id}
                );
                handler(true, 'Succesfully created test.', 200, {test: testCopy})(req, res);
              }
            });
          }
        });
